...
- **Scheduled clusters (Slurm)**: If the compute profile describes a scheduler-managed cluster (e.g. the MATS Slurm cluster), you are driving it remotely: write the code and the `sbatch` script locally under `experiments/exp-NNN/`, `rsync` them to the cluster, submit with `ssh <host> 'cd <dir> && mkdir -p logs && sbatch run.sbatch'`, poll `squeue` (sleeping between polls) until the job leaves the queue, then pull the results back and copy the full Slurm log into `run.log`. Never execute the workload in the SSH shell — that is the shared login node — and never fall back to a local GPU that the profile does not authorize. Keep all `#SBATCH` directives above the first command, set a realistic `--time`, and stay on the free partition the profile authorizes. See `docs/COMPUTE-MATS.md`.
