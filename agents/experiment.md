...
- **Scheduled clusters (Slurm)**: If the compute profile describes a scheduler-managed cluster (e.g. the MATS Slurm cluster), the shell you are in is a shared login node. Submit every GPU job and any sustained CPU work with `sbatch`, wait for it (`squeue -u $USER`, then the job's log), and copy the log into `run.log` — never execute the workload directly. Keep all `#SBATCH` directives above the first command, set a realistic `--time`, and stay on the free partition the profile authorizes. See `docs/COMPUTE-MATS.md`.
